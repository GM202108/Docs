...
In order to ensure that the input was properly typed, each step that used that input had to use a utility function, like [`util.types.toNumber()`](spectral/modules/_util_.md/#const-tonumber), to ensure that the value was cast to the proper type.
...
An example of a more complex `clean` function that returns an object with multiple fields is available in our [examples repo](https://github.com/-io/examples/blob/9ff35d3174c6d55b25381a9b0c33997aea5625f1/components/data-example/src/index.ts#L10-L27).
...
// highlight-next-line
import { ComponentTestHarness } from "@-io/spectral/dist/testing";
