...
To see an example of upgrading a component from Spectral 1.x to 2.x, [this commit](https://github.com/-io/examples/commit/411598bac4163c8a25b492a9275590b3fd7855a9) upgrades the "Format Name" example component from the [writing custom components](custom-components/writing-custom-components.mdx) article.

To start, update `@-io/spectral` in your `package.json` file and then run `npm install` or `yarn install`:
...
[Component versioning](components.mdx#component-versioning) is handled by the platform, so there's no need to pass in a `version` property in your `component` declaration.
You can remove it.
