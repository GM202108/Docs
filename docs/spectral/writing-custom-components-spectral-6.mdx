---
id: writing-custom-components-spectral-6
title: Writing Custom Components with Spectral 6.x
description: Create custom Prismatic components using the Prismatic SDK with Spectral 6.x
sidebar_label: Spectral 6.x Docs
---
...
const validateIpAddress = (value: unknown) => {
  const ipAddressRegex = /^(?:(?:2(?:[0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9])\.){3}(?:(?:2([0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9]))$/;
...
- `instanceState`, `crossFlowState` and `executionState` gives you access to [persisted state](custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state).
...
- [Spectral 6.x upgrade guide](spectral/spectral-7-upgrade-guide.mdx)
