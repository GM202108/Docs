...
The big change in Spectral 5.x is the introduction of [connections](spectral/modules/_index_.md/#const-connection), which allow you to package up all the information you need (endpoints, credentials, etc) into single inputs.
...
import { action, input, component } from "@-io/spectral";
...
import { action, input, component, connection } from "@-io/spectral";
import { acmeClient } from "@acme-inventory/client";
...
You can read more about connection development on our [Writing Custom Components](custom-components/writing-custom-components.mdx#adding-connections) article.
Check out [this section](custom-components/writing-custom-components.mdx#writing-oauth-20-connections) for information on upgrading an OAuth 2.0 connection.
