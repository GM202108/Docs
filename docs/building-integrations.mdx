...
After [creating a new integration](integrations.mdx#creating-a-new-integration) or selecting one from your list of integrations, you will find yourself in the **integration designer**.
...
1. The testing drawer lets you [run tests of your integration](testing-integrations.mdx), supply your integration with sample payloads, and see results of your integration test runs.
1. The version history drawer lets you publish a version of your integration, so it can be deployed to customers.
1. The majority of the page is taken up by the integration editor pane.
   ...
   You can also create multiple **flows** - each flow has a [trigger](integration-triggers.mdx) and series of steps to be executed.

<Screenshot
  filename="integrations/integration-designer.png"
  maxWidth="800px"
  alt=" integration designer highlighting configuration drawer, testing drawer, version history drawer, and integration editor pane"
/>

## Integration steps

Actions, like downloading a file from an [SFTP server](components/sftp.mdx) or posting a message to [Slack](components/slack.mdx), are added as **steps** of an integration.
...
The [integration triggers](integration-triggers.mdx) article details how triggers work, and how to invoke your integration.

### Adding steps to integrations

To add a step to an integration, click the <Icon>add_circle</Icon> icon underneath the trigger or another action.
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Choose component or action to add step in  app"
/>
...
<Screenshot
  ...
  alt="Change a step's action in  app"
  maxWidth="470px"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Example of step outputs as a list in  app"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Example of step outputs as a file in  app"
/>
...
[Custom Component Action Results](custom-components/writing-custom-components.mdx#action-results)
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Reference earlier step result as step input in  app"
/>
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Config variable inputs in  app"
/>
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Template inputs in  app"
/>

**For More Information**:
[Custom Component Inputs](custom-components/writing-custom-components.mdx#adding-inputs)
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Possible code change prompt in  app"
/>
...
<Screenshot
  ...
  maxWidth="350px"
  alt="Reorder flow steps in  app"
/>

### Choosing component versions

Components are [versioned](components.mdx#component-versions).
...
<Screenshot
  ...
  maxWidth="350px"
  alt="Component version list highlighting latest version in  app"
/>
...
**For More Information**:
[Component Versioning](components.mdx#component-versioning)
...
You can use the [branch](components/branch.mdx) component to branch based on that error.
...
Each flow has its own unique trigger, and so has its own [webhook URL](integration-triggers.mdx#webhook-triggers) that you can use to invoke that specific flow.
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Manage integration flows in  app"
/>
...
Then, [run tests](testing-integrations.mdx) just as you would with a single-flow integration.

## Config variables in integrations

When your customer configures and activates an instance of your integration, they'll walk through an instance [configuration wizard](configuration-wizard.mdx) where they'll enter authenticate with third-party apps, select options from dropdown menus, map fields, toggle boolean values, etc.
...
For information on building a configuration wizard experience for your users, see the [Configuration Wizard](configuration-wizard.mdx) article.

## Synchronous / asynchronous

Integrations can be triggered synchronously or asynchronously.
For more information, see the [integration triggers](integration-triggers.mdx#synchronous-and-asynchronous-integrations) article.
...
You can configure your [asynchronously](integration-triggers.mdx#synchronous-and-asynchronous-integrations)-invoked instances to retry if they fail to run to completion.
...
<!-- <Vimeo title="How to Configure an Integration to Retry" video="507722700" /> -->
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Set integration to retry in  app"
/>

**Retry Attempts** indicates the maximum number of times (up to 5) that  will run the same instance invocation in the event of failure.
If an instance has failed more than **Retry Attempts** number of times, the run will be marked as _execution failed_ and relevant [alert monitors](monitoring-and-alerting.mdx), if configured, will fire.

**Minutes Between Attempts** indicates the number of minutes that  should wait before trying to run an instance again.
...
**For More Information**:
[Instance Retry and Replay](instances.mdx#instance-execution-retry-and-replay)

## Looping

<!-- <Vimeo title="How to Loop Over Files in an Integration" video="502284421" /> -->
...
If your integration sends alerts to users, you might want to loop over an array of users.
 ...
 provides the [loop component](components/loop.mdx) to allow you to loop over an array of items, or you can loop a predetermined number of times.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Loop over files by referencing results of list files step in  app"
/>
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Loop over items to get file paths in  app"
/>

**For More Information**:
[The Loop Component](components/loop.mdx),
[Looping Over Files Quickstart](quickstarts/looping-over-files.mdx),
[Looping Over a Paginated API](quickstarts/looping-over-a-paginated-api.mdx)
...
That will log lines like `User #1: Bob Smith - bob.smith@.io`.
...
You can accomplish that with a combination of two loops (one to loop over pages, and one to loop over records on each page) and a [break loop](components/loop.mdx#break-loop) action that stops loading pages when there are no more left to load:

<Screenshot
  filename="integrations/paginated-loop.png"
  maxWidth="550px"
  alt="Loop over paginated API in  app"
/>

Please reference [this quickstart](quickstarts/looping-over-a-paginated-api.mdx) for an example of how to loop over a paginated API.
...
The [branch](components/branch.mdx) component allows you to add branching logic to your integration.
...
**For More Information**:
[The Branch Component](components/branch.mdx),

### Branching on a value

Adding a [Branch on Value](components/branch.mdx#branchonvalue) action to your integration allows you to create a set of branches based on the value of some particular variable.
It's very similar to the switch/case construct present in many programming languages.

<!-- <Vimeo
  title="How to Branch on a Single Value in an Integration"
  video="504463395"
/> -->
...
<Screenshot
  ...
  alt="Branch on value in  app"
/>

**For More Information**:
[Branch on Value Action](components/branch.mdx#branchonvalue)

### Branching on an expression

The [Branch on Expression](components/branch.mdx#branchonexpression) action allows you to create branches within your integration based on more complex inputs.
...
<!-- <Vimeo
  title="How to Branch on an Expression in an Integration"
  video="504487086"
/> -->
...
<Screenshot
  ...
  alt="Branch on expression in  app"
/>
...
The same logic can be represented with a group of conditionals in a [Branch on Expression](components/branch.mdx#branchonexpression) action:

<Screenshot
  filename="shared/branch-on-expression-logic.png"
  maxWidth="500px"
  alt="Branch on expression using conditionals in  app"
/>

**For More Information**:
[Branch on Expression Action](components/branch.mdx#branchonexpression)
...
In your integration you may want to say "if branch _foo_ was executed, get the results from _step A_, and if branch _bar_ was executed, get the results instead from _step B_."
 provides the [Select Executed Step Result](components/branch.mdx#select-executed-step-result) to handle that scenario.
...
You can leverage the [Select Executed Step Result](components/branch.mdx#select-executed-step-result) action to say "get me the incoming or outgoing invoice - whichever one was executed."
...
<Screenshot
  ...
  alt="Select executed step result while branching in  app"
/>
...
Sometimes it's helpful to save data from one execution of an integration so it can be used in a subsequent execution.
 provides components that allow you to save some state from one run for use in a future run.
...
You can persist "123" using a [Save Value](components/persist-data.mdx#flow---save-value) action, and then the next time your integration runs it can use [Get Value](components/persist-data.mdx#flow---get-value) to know that "123" was the most recently processed record.
...
Data can be persisted between runs using the [Persist Data](components/persist-data.mdx) component.
...
You can store a key/value pair using the [Save Value](components/persist-data.mdx#flow---save-value) action, or you can use [Persist Data](components/persist-data.mdx)'s other actions to append to a persisted list.
If you would like to save a timestamp instead, you can use the [Save Current Time](components/persist-data.mdx#flow---save-current-time) action to save the current time into a key of your choosing.

Later, in a subsequent run, you can fetch the value you saved using the [Get Value](components/persist-data.mdx#flow---get-value) action.
If a key is not set, **Get Value** will return `null`.

You can remove data from an array, or remove a key/value pair altogether, using [Persist Data](components/persist-data.mdx)'s other actions.
...
The Process Data's [DeDuplicate](components/process-data.mdx#deduplicate) action allows you to pass in such an array objects in **descending order**, along with a unique identifier (like `orderid`).
...
For More Information:
[Persisting Data in Custom Components](custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state)
...
To interact with a SOAP-based API, you can either use the built-in generic [SOAP component](components/soap.mdx) to fetch and interpret the WSDL and make requests to the API, or you can write your own custom component leveraging the  SDK's [SOAP utility functions](custom-components/writing-custom-components.mdx#soap-utility-functions).
