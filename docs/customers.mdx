...
id: customers
title: Customers
description: Create and manage Prismatic customers
---
...
Your [organization](organization.mdx) creates customers and [integrations](integrations.mdx) in .
Then, your customers can either enable instances of your integrations themselves through your [integration marketplace](integration-marketplace.mdx), or your team members can deploy [instances](instances.mdx) to them.
You can create [users](users.mdx#customer-users) for your customers and assign them [roles](users.mdx#customer-user-roles) so that they can manage and have insight into the instances that are deployed to them.
...
Like all  resources, customers can be managed via 's CLI tool, [prism](cli/cli-usage.mdx), or through the web app by clicking the **Customers** link on the left-hand sidebar.

<Screenshot
  filename="customers/customers-page.png"
  alt="List of customers in  app"
/>
...
Create a new customer using the [createCustomer](api/schema/mutation/createCustomer.mdx) mutation:
...
Query [customers](api/schema/query/customers.mdx) for a list of customers:
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Edit customer details in  app"
/>
...
Use the [updateCustomer](api/schema/mutation/updateCustomer.mdx) mutation to update a customer:
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Search customers by label in  app"
/>
...
For more information on attaching config variables, see the [instances](instances.mdx) page.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Deploy instance to a customer in  app"
/>
...
[Instances](instances.mdx) and
[Configuration-Driven Integration Quickstart](quickstarts/configuration-driven-integration.mdx#deploy-your-integration-to-customers)

## Managing alert monitors

**Alert monitors** allow you to send alerts to your team when an [alert trigger](monitoring-and-alerting.mdx#alert-triggers) fires on an instance.
...
<Screenshot
  ...
  alt="Alert monitors list for customer in  app"
/>
...
<Screenshot
  ...
  alt="Alert monitors for an instance in  app"
/>
...
To create an alert monitor for an instance, you will need to query [alertTriggers](api/schema/query/alertTriggers.mdx) and select which types of triggers should result in an alert:
...
Then, use the [createAlertMonitor](api/schema/mutation/createAlertMonitor.mdx) mutation to create the alert monitor:
...
At  we believe verbose and searchable logs are critical to any software ecosystem.
...
<Screenshot
  ...
  alt="Access customer instance logs in  app"
/>
...
<Screenshot
  ...
  alt="Search customer instance logs in  app"
/>
...
<Screenshot
  ...
  alt="Filter customer instance logs in  app"
/>
...
<Screenshot
  ...
  alt="Customer instance log entry details in  app"
/>

**For More Information**:
[Logging](logging.mdx),
[Log Retention](logging.mdx#log-retention),
[Logging from Custom Components](custom-components/writing-custom-components.mdx#logger-object)
...
<Screenshot
  ...
  alt="Logs and step outputs for customer instances in  app"
/>
...
Depending on the [role](users.mdx#customer-user-roles) you assign to the user, the user might have read or write access to their instances.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Manage customer users in  app"
/>
...
More information about customer user roles can be found on the [users](users.mdx#customer-user-roles) page.

<Screenshot
  filename="shared/add-customer-user.png"
  maxWidth="800px"
  alt="Add customer user in  app"
/>
...
More information about user roles can be found on the [users](users.mdx#customer-user-roles) page.
...
To list all customers query [customers](api/schema/query/customers.mdx), or query [customer](api/schema/query/customer.mdx) for a specific customer's users:
...
To create a new customer user, use the [createCustomerUser](api/schema/mutation/createCustomerUser.mdx) mutation.
...
Delete a user using the [deleteUser](api/schema/mutation/deleteUser.mdx) mutation:
...
<Screenshot
  ...
  alt="Add attachments to customers in  app"
/>
...
Delete a customer using the [deleteCustomer](api/schema/mutation/deleteCustomer.mdx) mutation:
...
So, if you know "Smith Rocket Company" as a customer with an ID of `abc-123` in another system you use, you can assign "Smith Rocket Company" in  the `externalId` of `abc-123`.
This is helpful if you need to quickly look up or associate customers in  with customers in your external system.

External IDs can be set programmatically (see the next section), or can be edited within the  we app by clicking the **Customers** link on the left-hand sidebar, selecting a customer, and then clicking on the **Details** tab.

<Screenshot
  filename="customers/external-id.png"
  maxWidth="700px"
  alt="Set customer external IDs in  app"
/>

External IDs are required if you want to route webhook requests to instances deployed to specific customers using [shared](integration-triggers.mdx#webhook-endpoint-configuration) webhook triggers.

### Sync customers from an external system programmatically

If you use a customer management tool to keep track of your customers, like Salesforce or another tool specific to your company or industry, you can use 's API to programmatically sync customers from your external tool to .

In addition at a **name** and **description**, a customer created through the [createCustomer](api/schema/mutation/createCustomer.mdx) mutation can be assigned an `externalId`, so that you can match customers in  to customers saved in other systems.

See our [Syncing Customers to ](quickstarts/syncing-customers-to-.mdx) quickstart guide for an example of how to sync customers to  using our API.
