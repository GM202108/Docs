...
## Escribir desencadenadores

Las integraciones suelen activarse [según un horario](integration-triggers.mdx#scheduled-triggers) (lo que significa que las instancias de la integración se ejecutan cada X minutos o en un momento específico del día) o [a través de un webhook](integration-triggers.mdx#webhook-triggers) (lo que significa que algún sistema externo envía datos JSON a una URL única y una instancia procesa los datos enviados).
La gran mayoría de las integraciones construidas en comienzan con un desencadenador de horario o un desencadenador de webhook estándar.
Sin embargo, hay situaciones en las que ni el horario ni el desencadenador de webhook estándar son adecuados por alguna razón.
Es aquí donde resulta útil escribir tus propios desencadenadores.

Los desencadenadores son fragmentos de código personalizados similares a [acciones](#writing-actions).
Te ofrecen un control detallado sobre cómo se presenta la carga útil de un webhook al resto de los pasos de una integración y qué respuesta HTTP se devuelve a quien invocó la URL del webhook del desencadenador.

Similar a una acción, un desencadenador se compone de información de `display`, una función `perform` e `inputs`.
Además, especificas si tu desencadenador puede ser invocado [sincrónicamente](integration-triggers.mdx#synchronous-and-asynchronous-integrations) (`synchronousResponseSupport`) y si tu desencadenador admite la programación (`scheduleSupport`).

Supongamos, por ejemplo, que una aplicación de terceros se puede configurar para enviar datos CSV a través de un webhook y requiere que el webhook devuelva un encabezado, `x-confirmation-code`, en texto plano para confirmar que recibió la carga útil.
El desencadenador de webhook estándar acepta JSON y responde con un [ID de ejecución](api/schema/object/InstanceExecutionResult.mdx#return-fields), por lo que no es adecuado para integrarse con esta aplicación de terceros.


Este desencadenador devolverá un HTTP 200 y devolverá un encabezado específico al sistema que invoca el webhook, y luego analizará la carga útil CSV en un objeto para que los pasos posteriores puedan hacer referencia a través de `results.body.data` del desencadenador:
...
export const csvTrigger = trigger({
  ...
  perform: async (context, payload, { hasHeader }) => {
    // Crear una respuesta HTTP personalizada que se haga eco de una cabecera,
    // x-confirmation-code, que se recibió como parte de
    // la invocación webhook
    ...
    // Crear una copia de la carga útil del webhook, deserializar
    // el cuerpo en bruto del CSV, y añade el objeto deserializado
    // al objeto a las salidas del disparador
    ...
    // Devuelve la carga útil modificada del disparador y la respuesta HTTP personalizada
  ...
  inputs: {
    // Declarar si el CSV entrante tiene información de cabecera
    hasHeader: input({
...
Fíjese en algunas cosas de este ejemplo:

- El formulario del `trigger` es muy similar al de la definición de una `action`.
- La `response` contiene un `statusCode` HTTP, `body` y `contentType` que se devolverán al llamador del webhook.
- El segundo argumento de la función `perform` - `payload` - contiene la misma información que devuelve un desencadenador de webhook estándar.
  El `rawBody.data` presume contener algún texto CSV; la clave `body.data` del payload se reemplaza por la versión deserializada de los datos CSV.
- `inputs` funcionan de la misma manera que funcionan para las acciones: defines una serie de `input`s y se pasan como el tercer parámetro de la función `perform`.

Para otro ejemplo del mundo real sobre cómo escribir un desencadenador, consulta nuestro tutorial sobre cómo [escribimos el desencadenador de Salesforce](quickstarts/writing-a-trigger.mdx).

### Eventos de implementación y eliminación de instancias para desencadenadores

Similar a una función `perform`, un desencadenador también puede definir funciones `onInstanceDeploy` y `onInstanceDelete`.
Estas funciones se llaman cuando se implementa o elimina una instancia, respectivamente.
Son útiles para crear o eliminar recursos en un sistema de terceros que están asociados con una instancia (como directorios de archivos, webhooks, etc.).


Este ejemplo de desencadenador creará un webhook en una aplicación de terceros cuando se implemente una instancia, almacenando el ID del webhook en datos persistentes, y eliminará el webhook cuando se elimine la instancia:
...
const acmeWebhookTrigger = trigger({
  display: {
    ...
    description: "Acme notificará a su aplicación cuando se produzcan determinados eventos en Acme",
  },
  ...
  inputs: {
    ...
    events: input({
      ...
      label: "Events",
      comments:
        "Los eventos que provocarían el envío de una solicitud de webhook de Acme a este flujo",
      collection: "valuelist",
      model: [
  ...
  /** Se ejecuta cuando se invoca un activador. Esta función podría contener lógica adicional para verificar firmas HMAC, etc. */
  perform: async (_context, payload, _params) => {
    return Promise.resolve({ payload });
  },

  /** Se ejecuta cuando se despliega una instancia con este activador */
  onInstanceDeploy: async (context, params) => {
    // Obtener la URL del webhook del flujo actual
    const flowWebhookUrl = context.webhookUrls[context.flow.name];

    // Crear un webhook en Acme
  ...
  /** Se ejecuta cuando se elimina una instancia con este disparador */
  onInstanceDelete: async (context, params) => {
    // Obtener el ID del webhook del estado de la instancia
    const webhookId = context.instanceState["webhookId"];

    // Borrar el webhook de Acme
...
:::tip Asegúrate de que tu función `onInstanceDeploy` sea idempotente. Ya sea que la API externa de terceros o tu desencadenador deben estar diseñados para ser idempotentes, lo que significa que si `onInstanceDeploy` se crea dos veces, no causará ningún problema.

:::

### Agregar un desencadenador a tu componente

Una vez que hayas escrito un desencadenador, puedes agregarlo a un componente existente de la misma manera que agregas una acción a tu componente, pero utilizando la clave `triggers`:
...
### Pruebas de un desencadenador

Probar un desencadenador es similar a [probar una acción](#pruebas-de-las-acciones-del-componente), excepto que utilizas la función [harness.trigger](spectral/classes/_testing_.componenttestharness.md#trigger).
Por ejemplo, si deseas utilizar Jest para probar el `csvTrigger` descrito anteriormente, tu prueba podría verse así:
...
Una **Fuente de Datos** obtiene datos de una API de terceros que se utilizarán para generar dinámicamente una variable de configuración.
Cuando tu cliente implementa una instancia, utilizan una [conexión](#agregar-conexiones) para autenticarse con una API de terceros.

Una fuente de datos puede generar una variedad de [tipos de datos](https://github.com/-io/spectral/blob/main/packages/spectral/src/types/DataSourceResult.ts) que incluyen un `string`, `date`, `picklist` (que es un `string[]`), objetos complejos de tipo `objectSelection` y más.

Aquí tienes una fuente de datos simple que obtiene una matriz de clientes, cada uno con un `nombre` e `id`.
Los mapea a un objeto `label`/`key` para que los nombres de los clientes se muestren en un `picklist`, y el `id` del cliente sea:


```typescript title="Obtener una cadena de una API externa"
...
En este ejemplo, obtenemos varios elementos de una API, incluidos los metadatos de cada elemento, de forma que un usuario pueda seleccionar uno o varios de los elementos y obtener los metadatos de cada uno:
...
Un ejemplo de una fuente de datos que genera un picklist está disponible en el [componente de Slack](https://github.com/-io/examples/blob/main/components/slack/src/dataSources.ts).


## Fuentes de datos para formularios JSON

[JSON Forms](https://jsonforms.io/) es un marco generador de formularios que te permite crear formularios a través de un esquema JSON que generas.
Un formulario JSON puede contener cualquier cantidad de entradas de cadena, booleanas, numéricas, de fecha, hora, fecha y hora o de enumeración (menú desplegable), y tienes cierto control sobre cómo se representan los elementos de entrada (en pestañas, agrupados, diseño vertical u horizontal, etc.).


La documentación completa de JSON Forms está disponible en su [página de documentación](https://jsonforms.io/examples), que incluye varios ejemplos.
Además, ofrece un [JSON Forms playground](https://jsonforms.io/playground/) donde puedes crear nuevos formularios y ver cómo se representarían.


Una fuente de datos de configuración de JSON Forms debe devolver dos propiedades (y una propiedad opcional):


- `schema` define los tipos de entradas que contiene tu formulario (sus `propiedades`), y algunos validadores opcionales, como qué propiedades son requeridas.
- `uiSchema` define cómo se deben representar esas entradas, como si las entradas deben estar alineadas vertical u horizontalmente.
- `data` (opcional) te permite especificar algunos valores predeterminados para las entradas de tu formulario.

En este ejemplo simple, el `schema` declara dos entradas: `name` (una cadena) y `age` (un entero entre 0 y 150), y el `uiSchema` etiqueta la primera entrada como "Nombre" y coloca los campos de entrada horizontalmente uno al lado del otro.


```typescript title="Un simple formulario JSON"
return {
  result: {
    schema: {
      type: "object",
...
El formulario JSON resultante tiene el siguiente aspecto:
...
Un caso de uso común para JSON Forms es presentar una interfaz de mapeo de datos a un usuario.
Para fines de ilustración, obtendremos [usuarios](https://jsonplaceholder.typicode.com/users) y [tareas pendientes](https://jsonplaceholder.typicode.com/todos?_limit=10) de [JSON Placeholder](https://jsonplaceholder.typicode.com/), y daremos a nuestros usuarios la capacidad de asignar cualquier número de usuarios a tareas.

Con el fin de proporcionar cualquier cantidad de asignaciones de usuario a tarea, nuestro esquema JSON deberá contener un [`array`](https://jsonforms.io/examples/array) de `object`.
Cada `object` contendrá una propiedad `user` y una propiedad `task`.
La propiedad `user` y la propiedad `task` tendrán cada una una propiedad [`oneOf`](https://jsonforms.io/docs/multiple-choice#one-of), que representa un menú desplegable.


Para fines de ilustración, también proporcionamos un valor `data` que contiene algunos valores predeterminados que nuestra interfaz de usuario debería mostrar.
Esta propiedad se puede omitir.


```typescript title="Mapeo de datos con formularios JSON"
...
const userTaskExample = dataSource({
  ...
  display: {
    ...
    description: "Asignar usuarios a tareas pendientes",
  },
  inputs: {},
  perform: async (context, params) => {
    ...
    const schema = {
      ...
      properties: {
        mymappings: {
          ...
          type: "array",
          items: {
            // Cada objeto de la matriz debe contener un usuario y una tarea
            type: "object",
            properties: {
              user: {
                type: "string",
                ...
                oneOf: users.map((user) => ({
                  // JSON Forms espera un valor de cadena:
    ...
    // Proporcionar un valor por defecto, la asignación del primer usuario a la primera tarea
    const defaultValues = {
      mymappings: [
        {
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Resultado del formulario JSON de asignación de datos"
/>

### Utilización de las conexiones de componentes existentes en las fuentes de datos

Es posible que desees ampliar un componente existente para completar una variable de configuración.
Por ejemplo, es posible que desees recuperar y filtrar información específica de un CRM o ERP y presentar los datos a tu usuario como un menú de selección.
Tu origen de datos puede hacer referencia a cualquier variable de configuración de conexión existente, incluidas aquellas de componentes integrados.

Para utilizar la conexión de un componente existente, haz referencia a los nombres clave de su conexión.
El componente [AWS Glue](components/aws-glue.mdx), por ejemplo, tiene un `accessKeyId` y un `secretAccessKey`.

Su fuente de datos puede hacer referencia a ellos con:
...
El campo que probablemente necesitas utilizar para las conexiones de OAuth 2.0 es el `access_token` de la conexión, que está anidado bajo `token` de la siguiente manera:
...
Un ejemplo de reutilización de conexiones existentes está disponible en el tutorial [Building a Field Mapper Data Source](quickstarts/field-mapping-json-forms-datasource.mdx), que aborda la extracción de campos personalizados de Salesforce.


## Bifurcación en acciones y activadores personalizados

Al igual que el componente [branch](components/branch.mdx), tus acciones y desencadenadores personalizados pueden hacer uso de ramificaciones lógicas.
Para admitir ramificaciones, proporciona a tu función `action()` o `trigger()` dos propiedades adicionales, `allowsBranching: true` y `staticBranchNames: ["List", "Of", "Branches"]`, y asegúrate de que el objeto que devuelve tu función `perform` incluya una propiedad `branch`:


```typescript {9,10,18} title="Ejemplo de acción con ramificación"
export const branchExample = action({
  display: {
    label: "Branch Example",
    description: "Una acción de ejemplo que ramifica",
  },
  inputs: {
...
Un código similar puede utilizarse en un desencadenador personalizado.
`allowsBranching: true` indica al diseñador de la integración que debe renderizar ramificaciones debajo de tu acción o desencadenador.
`staticBranchNames` es una matriz de cadenas que representan nombres de posibles ramificaciones que se pueden seguir.
Se seguirá la ramificación cuyo nombre coincida con el valor devuelto por `branch`.


<Screenshot
  filename="custom-components/writing-custom-components/custom-branch-logs.png"
  maxWidth="800px"
  alt="Registros que muestran qué rama se siguió en un paso de rama personalizado"
  caption="Los registros de ejecución siempre indicarán qué rama de un paso de ramificación se ha seguido."
/>

## Gestión de archivos binarios en componentes personalizados

En general, las integraciones procesan datos JSON serializados, XML u otras cadenas simples y pasan objetos JavaScript deserializados entre pasos.
Pero hay situaciones en las que es posible que desees procesar y pasar datos binarios entre pasos.
Con "datos binarios", nos referimos a archivos que no son texto plano: archivos PDF, imágenes, audio MP3, etc.

Dentro de una integración, un archivo binario se representa por su `contentType` (tipo MIME) y un `Buffer` que contiene los datos del archivo.
Consulta la documentación de Mozilla para obtener una lista de [tipos MIME](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types) comunes de archivos.
...
### Tratamiento de datos binarios como entrada

Las entradas para archivos binarios son similares a cualquier otra entrada que puedas crear, aunque puedes utilizar la función `util.types.toData` para asegurarte de que la entrada tenga la forma `{ data: Buffer, contentType: string }`:
...
La propiedad `myFile` que llega a tu función `perform` tendrá la forma de un archivo binario, con propiedades `data` y `contentType` a las que puedes hacer referencia.
...
```ts
{
  perform: async (context, params) => {
    ...
    // Enviar los datos a un endpoint
...
### Devolviendo datos binarios desde una acción

Para devolver un archivo binario desde tu acción, asegúrate de que los datos que devuelves sean un `Buffer` y, opcionalmente, incluye una propiedad `contentType` junto con `data` que indique su tipo MIME.
Por ejemplo, si tu componente personalizado devuelve un archivo PDF generado y el contenido del PDF está guardado en una variable `Buffer` llamada `pdfContents`, el bloque de retorno podría verse así:
...
Puede devolver varios archivos, o archivos binarios en un objeto anidado con una estructura similar:
...
### Obtención de datos binarios con el cliente Spectral HTTP

Al recuperar datos binarios de una API, debes configurar tu cliente HTTP para esperar datos binarios y escribir los datos en un `Buffer`.
Para el cliente HTTP (que está basado en Axios), utiliza la opción de configuración `responseType: "arraybuffer"` para asegurarte de que la propiedad `data` devuelta sea un `Buffer`:
...
Empaquetar un componente con `webpack` corriendo `npm run build` o `yarn build`:
...
Esto creará un directorio `dist/` que contiene tu JavaScript compilado y la imagen del ícono.
Ahora utiliza `prism` para publicar tu componente.
Si no tienes instalada la herramienta de línea de comandos de , `prism`, toma un momento para revisar la [página de descripción general de Prism](cli/cli-usage.mdx).

```bash
$ prism components:publish

Formato de nombre - Formatea el nombre de una persona dadas su nombre, segundo nombre y apellido.
¿Te gustaría publicar el Formato de nombre? (s/N): y
¡Se ha enviado correctamente el Formato de nombre (v6)! La publicación debería completarse en breve.

```

## Actualización de versiones espectrales

Lanzamos cambios menores, no disruptivos, en Spectral con frecuencia, y cambios mayores periódicamente.
Los cambios mayores se acompañan de incrementos mayores en la versión (1.x.x, 2.x.x, 3.x.x, etc.).

Para actualizar un componente desde una versión mayor anterior de Spectral a una nueva, consulta nuestras guías de actualización:
...
Si está creando un nuevo componente, le recomendamos encarecidamente que empiece con la última versión de Spectral.