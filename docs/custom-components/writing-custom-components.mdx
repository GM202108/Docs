...
description: Prismatic allows dev users to write custom API connectors and other components for their integrations. Learn more about creating custom components with Prismatic.
---
...
[Sample component code](https://github.com/-io/examples/tree/main/components) is referenced throughout this page.
...
[![Spectral NPM version](https://badge.fury.io/js/@-io%2Fspectral.svg)](https://www.npmjs.com/package/@-io/spectral)
...
You can leverage the [`util.types.toNumber()`](spectral/modules/_util_.md/#const-tonumber) utility function and `clean` property to ensure that the input is presented to the `perform` function as a number:
...
- `instanceState`, `crossFlowState`, `integrationState` and `executionState` gives you access to [persisted state](custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state).
...
- `user` is an object containing an `id`, `name`, `email` (their ID) and `externalId` of the customer user whose user-level config was used for this execution.
  This only applies to instances with [User Level Configuration](user-level-configuration.mdx).
...
If you need longer log lines, consider [streaming logs](logging.mdx#external-log-streaming) to an external log service.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Step results from an action in  app"
/>
...
You can see the source code for `createClient` in Spectral's [GitHub repo](https://github.com/-io/spectral/blob/main/packages/spectral/src/clients/http/index.ts).
...
You can use the [`ComponentTestHarness`](spectral/classes/_testing_.componenttestharness.md) class and [`createHarness`](spectral/modules/_testing_.md#const-createharness) helper function to test your actions.
...
In addition to step results, the test utility's [`invoke`](spectral/modules/_testing_.md#const-invoke) function returns an object, `loggerMock`, with information on what was logged during the action invocation.
...
You can then reference the `_CONNECTION_VALUE` environment variable in your Jest tests.

More info is in our [prism docs](cli/prism.mdx#componentsdevrun).
:::

## Testing components from the CLI

The [`prism` CLI tool](cli/cli-usage.mdx) provides two commands for testing custom components:
...
- `prism components:dev:run` fetches an integration's active connection and saves the fields as an environment variable so you can run [unit tests](#unit-tests-for-custom-components) and other commands locally.
  This is helpful, since many unit tests require an access token from a validated OAuth 2.0 connection - this provides a way of fetching the token from a connection you've already authenticated in the  integration designer.
- `prism components:dev:test` publishes your component under a temporary name, and runs a single-action test integration for you that tests the action.
  This is helpful for quickly testing an action in the real  integration runner environment.

### Access connections for local testing

The `prism components:dev:run` command fetches an active connection from the  integration designer, so you can use the connection's fields for unit testing.
The connection's values are set to an environment variable named `_CONNECTION_VALUE`, which can be used by a subsequent command.
...
Within your unit test code, you can use `harness.connectionValue()`, which pulls in the `_CONNECTION_VALUE` environment variable into a connection that you can use for tests:

```typescript title="Use _CONNECTION_VALUE for a Jest test"
import { createHarness } from "@-io/spectral/dist/testing";
...
// Parse the OAuth 2.0 connection from the _CONNECTION_VALUE environment variable
