...
To configure your integration's endpoint settings, click the **Endpoint Configuration** button on the right-hand side of the integration designer.
...
<Screenshot
  ...
  maxWidth="550px"
  alt="Select endpoint type in  app"
/>
...
Integrations that use this endpoint configuration often set up webhooks with a [deploy trigger](integration-triggers.mdx#instance-deploy-trigger), and remove them with an [instance remove](components/management-triggers.mdx#instance-remove) trigger.
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Set flow name from HTTP payload for Endpoint Configuration in  app"
/>
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Set flow name from HTTP header for Endpoint Configuration in  app"
/>
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Set flow name from preprocess flow for Endpoint Configuration in  app"
/>
...
If you do not use a preprocess flow, the shared endpoint's webhook invocation must include an [external customer ID](customers.mdx#customer-external-ids) and **flow name** either in the HTTP payload, or as HTTP headers.
...
```bash {2,4} title="Routing a request by header and payload"
curl https://hooks..io/trigger/EXAMPLE== \
  --location \
  --header "Content-Type: application/json" \
  --header "myflowname: Update Inventory" \
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Set flow name from HTTP payload or header for shared endpoint configuration in  app"
/>
...
The final step must return an object containing both a [external customer ID](customers.mdx#customer-external-ids) and a **flow name**.

<Screenshot
  filename="integrations/webhook-trigger-config/flow-name-and-customer-id-from-preprocess-flow.png"
  maxWidth="900px"
  alt="Set flow name with preprocess flow for shared endpoint configuration in  app"
/>
...
An integration can have [multiple versions](integrations.mdx#publishing-an-integration), and customers' instance can be on different versions.
...
You can test each of your flows individually (including a **preprocess flow**, if applicable) by clicking the **SAVE & TEST FLOW** button on the top right of the integration designer.
...
<Screenshot
  ...
  alt="Test endpoint configuration in  app"
/>
...
- **No API Keys** indicates that the flow can be invoked without an API key.
  This option is often paired with a trigger that handles security in another way (like with [HMAC](best-practices/secure-webhook-endpoints-hmac.mdx)).
...
<Screenshot
  ...
  maxWidth="350px"
  alt="Endpoint configuration security options in  app"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Endpoint configuration in config wizard in  app"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Add trigger details to config wizard in  app"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Trigger details in config wizard in  app"
/>
...
If you have an integration with [Instance-Specific Endpoint Configuration](#instance-specific-endpoint-configuration), then all logs and execution records will appear in the instance's [execution results](instances.mdx#viewing-instance-execution-results) page.
