...
title: Building an Advanced Component With a Connection
description: This quickstart examines how to build an advanced component with a connection
---

The goal of this quickstart is to examine the [Amazon S3 component](components/aws-s3.mdx) to get you accustomed to developing more advanced components that have multiple actions, handle credentials through connections, and handle non-string binary data input fields.

Amazon S3 component [source code](https://github.com/-io/examples/tree/main/components/aws-s3) is referenced throughout this quickstart.

## Prerequisites

You should read [Building Your First Custom Component](quickstarts/first-custom-component.mdx) and [Writing Custom Components](custom-components/writing-custom-components.mdx) prior to this quickstart to get a sense of project set up, and the `action`, `input`, `connection` and `component` objects that make up a component project.

## Reusable inputs

`input` definitions _can_ be written in an `action` code block. For example, the [Slack component](https://github.com/-io/examples/tree/main/components/slack) is comprised of a single action, and inputs are defined within that action.
...
Rather than wrapping the client creation logic into each action, authorization and client creation are placed in a distinct file, [auth.ts](https://github.com/-io/examples/blob/main/components/aws-s3/src/auth.ts), and referenced by the actions.
...
The [Slack component](components/slack.mdx), for example, will only ever post string messages.
...
If a reference to binary data is passed in (for example, from an [HTTP GET](components/http.mdx#httpget) action's output), `fileContents` will be an object with `.data` and `.contentType` properties.
