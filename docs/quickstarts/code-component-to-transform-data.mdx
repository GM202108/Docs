...
description: Using a code component to transform data
---

import Screenshot from "@site/src/components/screenshot";

's code component lets you incorporate custom JavaScript code anywhere in your integration.
The code component, alongside [custom components](custom-components/writing-custom-components.mdx), allows you to write the product- or industry-specific portions of your integration that aren't readily solved using built-in components from the [component catalog](components/component-catalog.mdx).

Both code and custom components have their use cases.
As a reminder here: code components are handy to write simple one-off, single integration code snippets.
If you need to run the same code for multiple integrations, if your code is complex enough that it would benefit from unit testing, or if you are reliant on lots of external Node.JS libraries, consider creating a [custom component](custom-components/writing-custom-components.mdx) instead.
...
We don't have time to wait for the other vendor to fix its software - our customer is clamoring for the new integration they paid for!
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Step outputs in  integration designer"
/>
