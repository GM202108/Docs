...
In this tutorial, we'll look at how to use the [loop component](components/loop.mdx) to loop over a paginated API.
...
First, we'll create a [Loop N Times](components/loop.mdx#loop-n-times) loop step to loop over pages.
...
1. A [Persist Data - Get Execution Value](components/persist-data.mdx#execution---get-value) step will help to track the `_start` value in our API request.
   ...
   <Screenshot
     ...
     alt="Persist Data - Get Execution Value in  integration designer"
   />

2. Next, we'll use the value from the previous step to make a call to JSON Placeholder.
   We'll add an [HTTP - GET Request](components/http.mdx#get-request) step and fetch `https://jsonplaceholder.typicode.com/posts?_limit=10&_start={{ THE VALUE WE GOT }}`:

   <Screenshot
     filename="quickstarts/looping-over-a-paginated-api/get-posts.png"
     alt="HTTP Get Request in  integration designer"
   />

### Are we done?

Next, we'll determine if we're done fetching posts.
We'll do this by adding a [Branch on Expression](components/branch.mdx#branch-on-expression) step, and we'll check to see if the "Get Posts" step we invoked returned an empty array:

<Screenshot
  filename="quickstarts/looping-over-a-paginated-api/branch-on-empty.png"
  alt="Branch on Expression to Check Empty in  integration designer"
/>

If the array returned was empty, we know there are no more results to page through.
In that case we'll add a [Break Loop](components/loop.mdx#break-loop) step to get out of our main loop:

<Screenshot
  filename="quickstarts/looping-over-a-paginated-api/break-loop.png"
  maxWidth="350px"
  alt="Break Loop in  integration designer"
/>
...
1. Add a [Loop Over Items](components/loop.mdx#loop-over-items) action that takes the results from the "Get Posts" step in order to loop over the results:
...
2. We'll "process" each post.
   ...
   <Screenshot
     ...
     alt="Log Write Message in  integration designer"
   />
...
1. We'll use a [Branch on Expression](components/branch.mdx#branch-on-expression) action to determine if the `currentItem` has `isLast=true` (which indicates if we're looping over the last post):

   <Screenshot
     filename="quickstarts/looping-over-a-paginated-api/is-last-post.png"
     maxWidth="400px"
     alt="Branch on Expression to check for last post in  integration designer"
   />

2. If we are on the last post, we'll use a [Persist Data - Save Execution Value](components/persist-data.mdx#execution---save-value) to save the ID of the last post.
   ...
   <Screenshot
     ...
     alt="Perist Data - Save Execution Value in  integration designer"
   />
