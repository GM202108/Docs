...
description: Learn to sync customers from an external system to Prismatic using our GraphQL API
---

As a B2B software company you likely rely on a customer or user management system like Salesforce or some in-house system to keep track of your customers.
In this tutorial, we'll cover how to sync customers from an external system to  using 's [GraphQL API](api/api-overview.mdx).
...
Full code shown in this tutorial can be found on [GitHub](https://github.com/-io/examples/blob/main/api/customers/customers.py).
...
Next, we'll import our necessary libraries and create a GraphQL client so we can run queries against 's API.
We'll define the transport layer for getting to our API (HTTP), which will include an authorization header to authenticate our client against :
...
This code assumes that an environment variable, `_API_KEY`, has been set (it's best practice not to hard-code API keys).
To get an API key into our environment variables, we can leverage the `me:token` [prism subcommand](cli/prism.mdx#metoken):

```bash
export _API_KEY=$(prism me:token)
```

Now that we have a working client, let's write some GraphQL queries and mutations to manage customers within :

## GraphQL queries and mutations

To sync customers, we'll use the [customers](api/schema/query/customers.mdx) query and [createCustomer](api/schema/mutation/createCustomer.mdx) and [deleteCustomer](api/schema/mutation/deleteCustomer.mdx) mutations.
...
First, let's write a function that gets a list of all of our customers, including their `name`, `description`,  `id`, and `externalId`:
...
This function runs a [customers](api/schema/query/customers.mdx) query against 's API.
We specify `isSystem: false` because your  tenant has a set of "system" customers that are used behind-the-scenes for running test integrations as you build them.
...
We could use the output of this function to figure out which customers have not been synced _into_ , or to sync  customers back to an outside system.

### GraphQL pagination

Before moving on, I want to address pagination.
By default, the  API returns the first 100 results for a query.
So, the [customers](api/schema/query/customers.mdx) query only returns 100 customers, even if we have more than 100 in the system.
...
The [customers](api/schema/query/customers.mdx) query allows you to filter customers by `externalId`.
...
To do that, we'll use the the [createCustomer](api/schema/mutation/createCustomer.mdx) GraphQL mutation.
...
The `createCustomer` mutation will return `errors` if the `name` or `externalId` you specified is already in the  system.
If the mutation does not throw an error, it will return a customer object containing the new customer's `name`, `description`, `externalId` and generated  `id`.
Let's try it out:
...
To do that, we'll use the [deleteCustomer](api/schema/mutation/deleteCustomer.mdx) mutation.
...
Then, we'll feed the  ID that we get into the `deleteCustomer` mutation:
