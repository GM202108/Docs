...
description: Learn how to make API requests to Prismatic's GraphQL API using the embedded SDK
---
...
For an example of querying the  API to generate a custom marketplace UI, see [GitHub](https://github.com/-io/embedded/blob/main/example-embedded-app/pages/examples/custom-ui-elements.tsx).

## The  GraphQL API

's API is GraphQL-based.
Full documentation on the API is available in the [API docs](api/api-overview.mdx), including several [example queries and mutations](api/using--api.mdx).
You can try out the  API using our [GraphiQL explorer tool](/explorer/).

## Embedded user scope and API permissions

When you authenticate a customer user in the embedded SDK, that user is attached to one of your customers in .
...
All examples use `.graphqlRequest`, which takes two parameters:

- `query` is a string that represents the query or mutation to send to the  API.
