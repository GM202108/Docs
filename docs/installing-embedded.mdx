...
To embed in your app, you must include some JavaScript code with your client application.
You have two choices for how to incorporate that code: you can add the [@-io/embedded](https://www.npmjs.com/package/@-io/embedded) NPM package to your web app Node.js project, or you can build the package from source and include it in your app through a `<script>` tag.
We strongly recommend installing the SDK via NPM so that you have intellisense and TypeScript support.

You can view the source code for the Embedded NPM package on [GitHub](https://github.com/-io/embedded).

### Installing embedded through NPM

[![Embedded NPM version](https://badge.fury.io/js/@-io%2Fembedded.svg)](https://www.npmjs.com/package/@-io/embedded)

If your web application is written in Node.js, you can include 's JavaScript code as a Node.js import.
Install the Embedded Node.js package from NPM with `npm` or `yarn`:
...
Then, for any page that needs to invoke a `.*` function, include the `@-io/embedded` package at the top of the file:
...
To install the SDK through a script tag, you will first need to build the `@-io/embedded` package.
You will need [Node.js](https://nodejs.org/) installed on your machine.

```bash title="Clone and build the  SDK"
# Clone the @-io/embedded repository from GitHub
...
# Build the @-io/embedded package
npm run build
```

This will create a directory in `-embedded/` called `dist/`, which contains `index.js` (along with some TypeScript files).
...
Once the script has loaded, `.*` functions (described below) are available for you to use.

You can also pull down the built `index.js` file from a JavaScript CDN like UNKPG: [unpkg.com/browse/@-io/embedded/](https://unpkg.com/browse/@-io/embedded/).

## Initialize the client

When is embedded in your app, it's presented in an [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe) that by default points to 's application URL (https://app..io).

To initialize the client, run the `init` function:

```javascript
.init();
```

If you use a [custom white-label domain](organization.mdx#custom-domains) (something like **https<!---->://integrations.my-example-company.com**), you can point the iframes that you embed your white-label domain instead.
...
One big advantage of embedding the marketplace or integration designer is that users don't need to be assigned an additional set of credentials to remember.
They can log in to your application, and you can provide them with an authentication token that allows them to deploy integrations.
You do this by generating a JSON Web Token (JWT) that is signed by a unique private key that you get from .
The JWT contains information about the user from your system.
When is presented a signed JWT, the JWT signature is verified and the user is granted a [customer role](users.mdx#customer-user-roles) of **Marketplace**, which allows them to deploy integrations for their specific customer in .

### JWT signing keys

Before you can generate a JWT, you will need a valid **signing key** from .
Within click on your organization name on the bottom of the left-hand sidebar, and then open the **Embedded** tab.
Click the **+ Add signing Key** button.
_Note_: You must be an [owner or admin](users.mdx#organization-team-member-roles) to create a signing key.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Get signing key in  app"
/>

:::note Private keys are not stored in
does not store the private signing key that is generated, and only saves the last 8 characters so you can easily match up a private key you have with one in our system.
...
- A **payload** with a few fields:
  ...
  - An optional `external_id` sets the external ID of the _user_ in .
  ...
  - The [external ID](customers.mdx#customer-external-ids) of the `customer` the user belongs to
  - A signing time (current time) `iat`
  - A `role` is only necessary for [user level configuration](user-level-configuration.mdx) (ULC).
...
A customer user with that identifier will be created in if it doesn't already exist, and will be granted permissions to configure and deploy instances to the customer they're assigned to (you assign the user to a customer in the examples below).
:::

Here are a couple of code snippets for JavaScript and Python that would create a valid JWT to authenticate a user in :
...
An example NextJS implementation of JWT generation is available in [GitHub](https://github.com/-io/embedded/blob/main/example-embedded-app/pages/api/-auth.tsx).

### Use the JWT to authenticate the user

Now that a user in your application has a signed JWT from the backend, you can authenticate them with the library using the `.authenticate()` function in your frontend application:
...
If your customer or organization ID in your JWT are incorrect, if your JWT is not signed correctly, or if the JWT is expired, `.authenticate()` will throw an error.

For an example React hook that wraps the `.authenticate()` function, see the [GitHub](https://github.com/-io/embedded/blob/main/example-embedded-app/src/useAuth.ts#L66).

#### Refreshing an embedded JWT

If a customer user's JWT expires, the customer user will see a 404 in their embedded iframe.
To reauthenticate a user prior to expiration, ensure that your frontend app fetches a new token for your user and then run `.authenticate({ token })` with the new token.
