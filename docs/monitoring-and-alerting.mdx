...
title: Integration Monitoring and Alerting
description: Get notified when an integration experiences problems. Use various monitors and alerts in Prismatic to notify your integration team of issues in real-time.
---
...
With properly configured monitoring and alerting you can put your mind at ease - no news is good news!
 ...
 alert monitors are configurable.
...
- An **alert monitor** is a combination of an alert group and some alert triggers, and is configured for an [instance](instances.mdx).
...
[Log Levels](logging.mdx#log-levels)
...
- `$SUBJECT` - The string literal ".io Alert"
...
- `$CUSTOMER_EXTERNAL_ID` - The [external ID](customers.mdx#customer-external-ids) of the customer the instance is deployed to
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Add HTTP headers to alert trigger in  app"
/>
...
To create an alert webhook, use the [createAlertWebhook](api/schema/mutation/createAlertWebhook.mdx) mutation:
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Edit alert webhooks in  app"
/>
...
Delete an alert webhook by alert webhook ID using the [deleteAlertWebhook](api/schema/mutation/deleteAlertWebhook.mdx) mutation:
...
Many operations teams use Slack to notify themselves of production issues.
 alert webhooks can be configured to send messages to a Slack channel.
...
Use the Slack webhook URL that you generated in a  alert webhook, and configure the payload template to read similar to this:
...
Once you have user IDs and alert webhook IDs, create an alert group using the [createAlertGroup](api/schema/mutation/createAlertGroup.mdx) mutation:
...
Delete an alert group using the [deleteAlertGroup](api/schema/mutation/deleteAlertGroup.mdx) mutation:
...
An **alert monitor** is a combination of an [alert group](#alert-groups) (users and webhooks) and an [alert trigger](#alert-triggers) that is configured for an [instance](instances.mdx).
...
Note that if your instances are configured to use a [shared endpoint](endpoint-configuration.mdx#shared-endpoint-configuration) and a **preprocess flow**, an alert monitor cannot be assigned to the preprocess flow since the preprocess flow runs independently of any deployed instance.
:::
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Create alert monitor in  app"
/>
...
<Screenshot
  ...
  alt="Configure alert monitor in  app"
/>
...
Click the **Explore > Monitors** link on the left-hand sidebar.
...
Delete an alert monitor with the [deleteAlertMonitor](api/schema/mutation/deleteAlertMonitor.mdx) mutation:
...
<Screenshot
  ...
  alt="Alert monitor indicator on instance in  app"
/>
...
<Screenshot
  ...
  alt="Instance monitors with current uncleared alerts highlighted in  app"
/>
...
<Screenshot
  ...
  alt="Logs for time adjacent to alert events in  app"
/>
...
List alert events for a particular alert monitor by querying [alertEvents](api/schema/query/alertEvents.mdx).
