...
The [code component](components/code.mdx) allows you to execute product or industry-specific code within an integration.
This page outlines when and how to use a code component.

<!-- <Vimeo
  video="500210809"
  title="How to Transform Data with the Code Component"
/> -->

## Why use the code component?

You will likely have integration logic that can't be solved using the [standard components](components/component-catalog.mdx)  provides.
The portion of your integrations that are specific to your product or industry can be accomplished using code component steps, or [custom components](custom-components/writing-custom-components.mdx).
...
Within the integration designer, [add a step](building-integrations.mdx#adding-steps-to-integrations) to your integration.
...
<Screenshot filename="code/code-step.png" alt="Code editor in  app" />
...
`context.configVars` provides the [Code Component](components/code.mdx) with access to all [config variables](building-integrations.mdx#config-variables-in-integrations), including connections, associated with the integration.
...
[Connections](connections.mdx) are a special type of config variable.
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Reference integration trigger payload in  app"
/>
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Test runner step results in  app"
/>
...
<Screenshot
  ...
  alt="Use output for prior step for input to new step in  app"
/>

To see an example of returning more complex data structures, and a good example use case for a code component, see the [Using a Code Component to Transform Data](quickstarts/code-component-to-transform-data.mdx) quickstart.

### Returning binary data from a code step

<!-- <Vimeo
  title="How to Handle Binary Files with the Code Component"
  video="502750058"
/> -->
...
To see an example use case for returning binary data from a code component, check out our [Generate a PDF with a Code Component](quickstarts/generating-a-pdf-with-a-code-component.mdx).
...
If you need external dependencies, we strongly recommend using a code step for prototyping, but building a [custom component](custom-components/writing-custom-components.mdx) for production use.
...
's SDK, [@-io/spectral](https://www.npmjs.com/package/@-io/spectral) is automatically imported into each code block as `spectral`.
...
A list of all util type functions is available in the [Spectral SDK docs](spectral/modules/_util_.md).
