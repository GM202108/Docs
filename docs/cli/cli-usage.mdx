---
id: cli-usage
title: How to Use CLI Tool
description: Installing the prism CLI tool allows users access to our API via the command line. You can also run CLI commands and get contextual help.
---

The CLI tool allows you to interact with the API programmatically so you can build, deploy, and support integrations from the command line.
The CLI tool is built using the [ API](api/api-overview.mdx), so any action that can be completed through the web app or API can be completed through the CLI tool, as well.

## Installing the CLI Tool

[![Prism NPM version](https://badge.fury.io/js/@-io%2Fprism.svg)](https://www.npmjs.com/package/@-io/prism)

's CLI tool, `prism`, is available at https://www.npmjs.com/package/@-io/prism and can be installed using `npm` or `yarn`:

```bash
npm install -g @-io/prism
# OR
yarn global add @-io/prism
```

Prism's source code is available on [GitHub](https://github.com/-io/prism), and serves as a great example for how to wrap the [ API](api/api-overview.mdx).
...
This will open a web browser for you to authenticate with your credentials.
...
See [Integrations in Multiple Regions](integrations-multiple-regions.mdx#logging-in-to-additional-regions-with-prism).
:::

## Running CLI Commands and Getting Help

All CLI commands generally follow the form `prism COMMAND`.
...
A full list of `prism` commands can be found on the [ CLI Command Reference](cli/prism.mdx) page.
...
Build, deploy, and support integrations in  from the comfort of your command line

VERSION
  @-io/prism/4.2.6 darwin-arm64 node-v14.17.6
...
All types of resources (customers, components, integrations, instances, actions, etc.) have `:list` subcommands.
...
Multiple `prism` commands can be strung together to manage resources.
...
Refresh tokens do _not_ expire, and are used to generate short-lived access tokens for 's API.
...
If you're working with [regions](integrations-multiple-regions.mdx) other than the default US commercial region, you can also specify an endpoint:
...
If your app uses a white-labeled domain (like integrations.my-company.com), you can use that endpoint instead for `_URL`.
