...
If your integration has multiple [flows](building-integrations.mdx#flows-in-integrations), each flow has its own trigger (and so its own webhook URL or schedule).
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Set static integration trigger in  app"
/>
...
<Screenshot
  ...
  maxWidth="700px"
  alt="Configure integration trigger to use config variable in  app"
/>
...
<Screenshot
  ...
  maxWidth="550px"
  alt="Set config variable for integration trigger in  app"
/>
...
Webhook triggers allow you to run a particular instance or [flow](building-integrations.mdx#flows-in-integrations) of an instance by making an HTTP POST, PUT, PATCH, DELETE or GET request to the webhook's URL.
...
The outside application can assemble some data, and send that data to a  webhook URL via an HTTP request.
...
<Screenshot
  ...
  alt="Set webhook trigger via POST request in  app"
/>
...
The response body contains an execution ID, which can be used later to get logs and step results from the  API.
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Customize webhook trigger response code in  app"
/>
...
When your webhook trigger is invoked [synchronously](#synchronous-and-asynchronous-integrations), your HTTP client sends a request and waits for the integration to finish running before closing the request.
 responds to the request with an HTTP 303 redirect, redirecting your HTTP client to an object in an Amazon S3 bucket that contains the results of the last step of your integration.
...
For information on configuring and troubleshooting webhook endpoints, see our [Endpoint Configuration](endpoint-configuration.mdx) article.
