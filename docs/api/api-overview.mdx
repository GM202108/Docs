...
description: Prismatic's API allows users to build, deploy and support integrations programmatically. Learn how to get started with our GraphQL API.
---
...
import useBaseUrl from "@docusaurus/useBaseUrl";
 ...
 provides a [GraphQL](https://graphql.org/)-based API for you to build, deploy, and support your integrations programmatically.
While we recommend that new users use the web app or [ CLI tool](cli/cli-usage.mdx) to manage  resources, developer users will likely want to use the API to script out integration management.

If you're currently logged in, you can [explore](/explorer) the  API with the GraphiQL explorer.
...
- **Reference an API specification** to sculpt your queries.
  GraphQL APIs offer a [schema](#s-graphql-schema), so you know what fields are available for what resources, and how the resources are interrelated.

## Why  Chose GraphQL

 chose to use GraphQL because of its power and flexibility.
...
GraphQL allows  backend developers and API consumers to work independently of one another.
...
## Querying 's API with the GraphiQL Explorer

's <a href={useBaseUrl('explorer')}>GraphiQL explorer</a> lets you test queries against the  API.
If you are logged in to the web app, you can perform authenticated queries against 's GraphQL API using the GraphiQL explorer.

<Screenshot
  filename="reference/-api/graphql-explorer.png"
  alt="Screenshot of the  GraphiQL Explorer Tool"
/>

## 's GraphQL Schema

The left-hand sidebar of this page contains information about 's GraphQL schema, its operations, and schema-defined types.
...
<Screenshot
  filename="reference/-api/graphql-docs.png"
  alt="Screenshot of the  GraphiQL Schema Tool"
/>
...
You will probably want to query the  API with tools outside of the GraphiQL explorer.
To do that, you'll need an API token.
When you authenticate against  through the web app or  CLI tool, your web browser or CLI tool receives a [JWT](https://jwt.io/) that can be used to query the API.

To view your token in the web browser, visit [https://app..io/get_auth_token/](https://app..io/get_auth_token/) while logged in.

If you are using the  CLI tool, you can use the subcommand `me:token`.
...
## Querying 's API with Another GraphQL Client
...
For API endpoint, enter `https://app..io/api`.

## Querying 's API Programmatically
...
In the example below we query the API for all  [components](components.mdx) and their labels, descriptions, keys, and whether authorization is required.
...
const apiEndpoint = "https://app..io/api/";
...
If you would like to set up a CI/CD pipeline or any other headless system to periodically query the  API, you will need to provide that system with a **refresh token**.
Your system will then exchange the **refresh token** for an **access token** that it can use to query the  API.
...
In this example, assume that we've created an environment variable, `_REFRESH_TOKEN` with the value we found on our CI/CD server:

```bash
_REFRESH_TOKEN="2uSiGgplFXAN_igEtOGPIpj3UcGuG0FADIljgJEXAMPLE"
```
...
```bash
RESPONSE=$(curl "https://app..io/auth/refresh" \
            --request POST \
            --data '{"refresh_token":"'${_REFRESH_TOKEN}'"}' \
            -H "Content-Type: application/json")

_ACCESS_TOKEN=$(echo ${RESPONSE} | jq -r .access_token)
```

That `_ACCESS_TOKEN` can now be used in place of the `API_KEY` in the [example above](#querying-s-api-programmatically).
That token can be used for 24 hours before needing to refresh again.
...
You can query the  API **20 times per second**.
