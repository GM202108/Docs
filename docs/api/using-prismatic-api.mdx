---
id: using-prismatic-api
title: Common API Queries and Mutations
description: Examples of how to use Prismatic's API to manage customers and instances
---

import GraphqlExample from "@site/src/components/graphql-example";

As you incorporate  into your development ecosystem, it's handy to be able to wrap 's GraphQL-based API so you can programmatically manage users, customers, instances, monitors, alerting and more.
On this page you will find examples of how to wrap common  GraphQL queries and and mutations.

This page doesn't cover every query or mutation that  supports, but should give you a good idea of how to do standard CRUD (create, read, update, delete) operations using the  API.

Before you begin, please read through our [Intro to 's API](api/api-overview.mdx) article and generate an [API token](api/api-overview.mdx#how-do-i-get-an-api-authorization-token) for yourself, so you can make API calls.

:::tip Reference the prism CLI tool's source code
's CLI tool, [prism](cli/cli-usage.mdx) is open-source and wraps the  API.
Reference its source code on [GitHub](https://github.com/-io/prism) to see examples of interacting with customers, integrations, components, etc. programmatically.
:::

## Managing Customers Through the API

Let's start by managing [customers](customers.mdx) programmatically.
If you'd like to see examples of these queries and mutations in code, check out this [python script](https://github.com/-io/examples/blob/main/api/customers/customers.py) that wraps many of the customer-related queries and mutations.

### Querying Customers Through the API

If you know the ID of the customer you would like to query, use the [customer](api/schema/query/customer.mdx) query.
...
If you would like information about several customers, use the [customers](api/schema/query/customers.mdx) query instead:
...
To add a new customer to your account, use the [createCustomer](api/schema/mutation/createCustomer.mdx) _mutation_.
...
You can update a customer using the [updateCustomer](api/schema/mutation/createCustomer.mdx) mutation.
...
The [deleteCustomer](api/schema/mutation/deleteCustomer.mdx) mutation takes a customer ID, and returns the customer (if it has been deleted), or an error if the customer can't be found.
...
If you're using the embedded marketplace and [signed JWTs](embedding-marketplace.mdx#authenticating-users) to seamlessly authenticate customer users, you probably won't need to manage your customer users programmatically.
...
First, look up the **roles** you can assign a user using either the [customerRoles](api/schema/query/customerRoles.mdx) or [organizationRoles](api/schema/query/organizationRoles.mdx) query:
...
Next, run the either the [createCustomerUser](api/schema/mutation/createCustomerUser.mdx) or [createOrganizationUser](api/schema/mutation/createOrganizationUser.mdx) mutation to create the user:

<GraphqlExample example="createOrganizationUser" />

Users who are created in this way are immediately sent an email asking them to set a password for .

### Listing Users by Customer Through the API

One big advantage of GraphQL is that you can query resources for _related_ data.
Once again we'll use the [customers](api/schema/query/customers.mdx) query to list each customer's **ID** and **name**, but in addition we'll list the users attached to the customer.
...
You can accomplish the other CRUD operations (update and delete) using the [updateUser](api/schema/mutation/updateUser.mdx) and [deleteUser](api/schema/mutation/deleteUser.mdx) mutations, similar to how you [update or delete customers](#updating-a-customer-through-the-api)
...
You can use the [integrations](api/schema/query/integrations.mdx) query to list integrations.
...
The [createInstance](api/schema/mutation/createInstance.mdx) mutation requires four pieces of information:

- The **ID** of a customer this instance is for. This can be found through the [customers query](api/schema/query/customers.mdx).
...
This query returns some additional data about the instance, including a list of [flows](building-integrations.mdx#flows-in-integrations) and their respective webhook URLs for invoking the flows.
...
- `updateInstance` lets you update multiple properties of an instance (like name, description, config variables, etc.).
  ...
  When you programmatically set config variables using the [updateInstance](api/schema/mutation/updateInstance.mdx) mutation, all config variables must be given a value.
  If you omit a config variable, that config variable will be set to its default value (even if it was previously set).

  If you'd like to set just a subset of config variables while leaving the others alone, use the [updateInstanceConfigVariables](api/schema/mutation/updateInstanceConfigVariables.mdx) mutation, described [below](#updating-config-variables-with-updateinstanceconfigvariables).
  :::
...
Before the [updateInstance](api/schema/mutation/updateInstance.mdx) mutation, let's query the instance for its current config variables.
...
If your config variable is a [connection](building-integrations.mdx#connection-config-variables), which usually represents a username, password, API key, OAuth 2.0 connection, the shape is a bit different. It'll be comprised of a set of `inputs` that have a `name`, `type`, and `value`.
...
If you are migrating to  from your own service or another integration platform, you may want to import your customers' OAuth 2.0 access and refresh tokens so that they do not need to re-authenticate.
...
An instance needs to be [deployed](instances.mdx#deploying-instances) in order for your new configuration to take effect.
To deploy an instance, use the [deployInstance](api/schema/mutation/deployInstance.mdx) mutation:

<GraphqlExample example="deployInstance" />

## Fetching Step Results Through the API

When you invoke an integration [asynchronously](integration-triggers.mdx#synchronous-and-asynchronous-integrations), an `executionId` is returned:
...
You can use the [executionResult](api/schema/query/executionResult.mdx) along with that `executionId` to fetch the results of that execution.
...
To account for the variety of things that a step can return,  uses [msgpack](https://msgpack.org/) to package up step results.
