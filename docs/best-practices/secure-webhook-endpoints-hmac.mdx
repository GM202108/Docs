...
description: Here's a simple way to ensure that only authorized systems can send data to webhook endpoints.
---

When it comes to transferring data via integrations, security is a top concern. To secure data being passed via webhooks ([for event-driven integrations](/docs/best-practices/common-integration-patterns/#event-driven-integrations)), you have a few options, including API keys, your own authorization headers, or HMAC.

recommends using HMAC because it's about the the simplest (and stoutest) approach you could use. It doesn't require that your team learn a new language or gain an advanced understanding of encryption, but it does an excellent job of protecting the integrity of your data at the point of transfer.
...
Setting up a HMAC for one of your integrations should be straightforward. To make things even simpler, we've added an [HMAC Webhook trigger](/docs/components/hash/#hmac-webhook-trigger) to our built in Hash component.

In many cases, this will address your needs, but for the times it doesn't, here is a quickstart tutorial for [Writing a Custom Webhook Trigger with HMAC Validation](/docs/quickstarts/custom-trigger-with-hmac/).
