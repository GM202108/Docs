...
id: users
title: Users
description: Manage Prismatic organization and customer users
---
...
Two types of users can log into  systems:
...
List users by querying the `users` field on [organization](api/schema/query/organization.mdx):
...
Once you have that role ID, use the [createOrganizationUser](api/schema/mutation/createOrganizationUser.mdx) mutation to create a new organization user:
...
To delete an organization team member, use the [deleteUser](api/schema/mutation/deleteUser.mdx) mutation:
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Add third-party team member in  app"
/>
...
<Screenshot
  ...
  alt="Set granular access for third-party team member in  app"
/>
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Add specific permissions to third-party team member in  app"
/>
...
That way, they can test invoking an integration in  from their third-party service and can verify that the data the integration receives is in the format you agreed upon.
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Select integration for third-party permissions in  app"
/>

On the next screen select the types of permissions you would like to grant for that integration.
If you would like to give read-only access to your integration, select **View Integration**.
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Choose integration permissions for third-party team member in  app"
/>
...
By default, third-party users have access to  built-in public components, but you may not want third-party vendors to see all of the custom components you've published (especially if you integrate with several competing vendors).
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Select component for third-party permissions in  app"
/>
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Choose component permissions for third-party team member in  app"
/>
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Select customer for third-party permissions in  app"
/>
...
- A customer that is created through an [Embedded Marketplace](embedding-marketplace.mdx) is automatically assigned the **marketplace admin** role, and is permitted to deploy and manage integrations that are deployed to them.
  A **marketplace user** only has the ability to configure their own [User level configuration](user-level-configuration.mdx) for an instance.
  Note: **marketplace** users are not assigned credentials and cannot log in to , except through the embedded integration marketplace.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Filter customer users in  app"
/>

</TabItem>
<TabItem value='cli'>

To list users with the  CLI tool, use the `prism customers:users:list` subcommand.
...
<Screenshot
  ...
  maxWidth="450px"
  alt="Delete customer user in  app"
/>
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Search all customer users in  app"
/>
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Change own password in  app"
/>

### Resetting a forgotten password

If you have forgotten your password, navigate to https://app..io.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Set light/dark mode in  app"
/>
...
## Syncing customer users to  programmatically

Similar to [syncing customers programmatically](customers.mdx#sync-customers-from-an-external-system-programmatically), if you manage customer users in an external system you can sync those users to  programmatically.
Though this tutorial is about syncing customers to , the concepts are similar for syncing users: [Syncing Customers to ](quickstarts/syncing-customers-to-.mdx).

To list customer users, use the [users](api/schema/query/users.mdx) query, and give it the argument `customer_Isnull: false` to indicate that you want _customer_ users:
...
To create new users, look up a customer's ID and the **role** that you want the user to have, and then invoke the [createCustomerUser](api/schema/mutation/createCustomerUser.mdx) mutation along with the user's `email`, `name`, and optional `phone` number.
You can also specify an `externalId` if you would like users in  to easily correspond to users in an external system.

To update customer users, use the [updateUser](api/schema/mutation/updateUser.mdx) mutation, and to delete a user use the [deleteUser](api/schema/mutation/deleteUser.mdx) mutation.
